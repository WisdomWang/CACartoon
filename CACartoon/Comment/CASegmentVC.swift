...
class CASegmentVC: UIViewController {
    ...
    var titleColor = UIColor.black //title默认颜色
    var titleSelectedColor = UIColor.red //title选中颜色
    ...
    extension CASegmentVC:UIScrollViewDelegate{
        func setupAllTitle() {
            ...
            for i in 0 ..< count {
                ...
                contentScrollView.contentSize = CGSize.init(width: CGFloat(count) * xScreenWidth, height: 0)
    ...
    func refreshFrame() {
        if isHave_Navgation == false {
            ...
            titleScrollView.frame = CGRect.init(x: 0, y: navH - 44, width: xScreenWidth, height: titleScrollViewH)
        }
        else{
            titleScrollView.frame = CGRect.init(x: 0, y: navH, width: xScreenWidth, height: titleScrollViewH)
        }
        let y : CGFloat = titleScrollView.frame.maxY + 1
        if isHave_Tabbar == true {
            contentScrollView.frame = CGRect.init(x: 0, y: y, width:xScreenWidth, height: xScreenHeight - y - 50)
        }
        else{
            contentScrollView.frame = CGRect.init(x: 0, y: y, width:xScreenWidth, height: xScreenHeight - y)
        }
        titleViewBottomLine.frame = CGRect.init(x: 0, y: titleScrollView.frame.maxY, width: xScreenWidth, height: 1)
    }
    //设置按钮下划线
    func setTitleBtnBottomLine(){
        titleBtnBottomLine.backgroundColor = UIColor.red
        titleScrollView.addSubview(titleBtnBottomLine)
    }
    //设置titleView下划线
    func setTitleViewBottomLine(){
        titleViewBottomLine.backgroundColor = UIColor.gray
        self.view.addSubview(titleViewBottomLine)
    }
}

//MARK:-  处理关联事件
extension CASegmentVC{
    @objc func titleBtnClick(titleBtn: UIButton)  {
        setupOneViewController(btnTag :titleBtn.tag)
        selButton(btn: titleBtn)
        let x :CGFloat  = CGFloat(titleBtn.tag) * xScreenWidth;
    ...
    func setupOneViewController(btnTag: NSInteger){
        if btnTag < count{
            ...
            let x : CGFloat = CGFloat(btnTag) * xScreenWidth
            VC.view.frame = CGRect.init(x: x, y: 0, width: xScreenWidth, height: contentScrollView.bounds.size.height)
            self.contentScrollView.addSubview(VC.view)
        }
    }
    func setupTitleCenter(btn: UIButton){
        var offsetPoint : CGPoint = titleScrollView.contentOffset
        offsetPoint.x =  btn.center.x -  xScreenWidth / 2
        //左边超出处理
        if offsetPoint.x < 0{
            offsetPoint.x = 0
        }
        let maxX : CGFloat = titleScrollView.contentSize.width - xScreenWidth;
        //右边超出处理
...
extension CASegmentVC{
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //获取标题
        let leftI :NSInteger   = NSInteger(scrollView.contentOffset.x / xScreenWidth);
        ...
        var scaleR :CGFloat  = scrollView.contentOffset.x / xScreenWidth;
